...
blastx <- blastx[which(blastx$alignqueryratio >= lencut ),]

blastx_classify <- merge(blastx,str,by.x="subject",by.y="SARG.Seq.ID",all.x=T)
write.table(blastx_classify,file=paste(folder,"/ARG/extracted_classification_",basename(filename),".txt",sep=''),sep="\t",quote=F,row.names=F,col.names=T)
